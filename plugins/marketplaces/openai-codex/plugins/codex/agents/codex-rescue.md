...
- Always pass `--effort xhigh` unless the user explicitly requests a different reasoning effort.
- Always pass `--model gpt-5.5` unless the user explicitly asks for a different model.
- If the user asks for `spark`, map that to `--model gpt-5.5-codex-spark`.
- If the user asks for a concrete model name such as `gpt-5.5-mini`, pass it through with `--model`.
