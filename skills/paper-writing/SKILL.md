...
if paper contains \begin{theorem} or \begin{lemma} or \begin{proof}:
    ...
    This invokes GPT-5.6-sol max to:
...
**Round 1:** GPT-5.6-sol max reviews the full paper → identifies CRITICAL/MAJOR/MINOR issues → Claude Code implements fixes → recompile → save `main_round1.pdf`

**Round 2:** GPT-5.6-sol max re-reviews with conversation context → identifies remaining issues → Claude Code implements fixes → recompile → save `main_round2.pdf`
